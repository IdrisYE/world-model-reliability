...
from src.models.tssm import TSSM, TSSMState
...
def parse_args():
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("checkpoints/tssm.pt"),
    )
...
def main():
    ...
    print(
        f"state mean={train_set.state_mean}"
    )

    print(
        f"state std={train_set.state_std}"
    )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for states, actions in train_loader:
            ...
            for t in range(args.seq_len - 1):
                ...
                prior_delta = model.predict_delta(
                    deter,
                    prior_state,
                )
                ...
                posterior_delta = model.predict_delta(
                    deter,
                    posterior_state,
                )
                ...
                # Posterior becomes state at t+1.
                tssm_state = posterior_state

            num_pred_steps = args.seq_len - 1

            recon_loss /= num_pred_steps
            prior_pred_loss /= num_pred_steps
            dyn_loss /= num_pred_steps
            rep_loss /= num_pred_steps
        ...
        train_recon = (
            train_recon_total / train_batches
        )

        train_prior = (
            train_prior_total / train_batches
        )
